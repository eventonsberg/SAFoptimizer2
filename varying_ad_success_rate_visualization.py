...
def plot_remaining_production_capacity_vs_ad_success_rate():
    results = st.session_state.varying_ad_success_rate_results
    ad_success_rates = []
    remaining_capacities = []
    for P_A, result in results.items():
        ad_success_rates.append(P_A)
        remaining_capacities.append(result["remaining_production_capacity_after_attack"])
    df = pd.DataFrame({
        "Suksessrate for luftvern": ad_success_rates,
    ...
    chart = alt.Chart(df).mark_line(
        ...
        x=alt.X(
            "Suksessrate for luftvern:O",
            title="Suksessrate for luftvern",
        ...
        tooltip=["Suksessrate for luftvern", "Produksjonskapasitet"],
    )
    st.altair_chart(chart)

def plot_facility_configuration_vs_ad_success_rate():
    type_f = st.session_state.varying_ad_success_rate_params["type_f"]
    results = st.session_state.varying_ad_success_rate_results
    data = []
    for P_A, result in results.items():
    ...
    bar = alt.Chart(df).mark_bar(
        ...
        tooltip=["Suksessrate for luftvern", "FabrikkID", "Luftvern", "Ødelagt"],
    )
    text = (
        alt.Chart(df).transform_window(
            stack_index='row_number()',
            groupby=["Suksessrate for luftvern", "Fabrikktype"],
            sort=[
            ...
            x=alt.X("Suksessrate for luftvern:O"),
            xOffset=alt.XOffset("Fabrikktype:N"),
            y=alt.Y(
                "LabelPos:Q"
            ),
...
def plot_costs_vs_ad_success_rate():
    C_f = st.session_state.varying_ad_success_rate_params["C_f"]
    C_A = st.session_state.varying_ad_success_rate_params["C_A"]
    B_B = st.session_state.varying_ad_success_rate_params["B_B"]
    results = st.session_state.varying_ad_success_rate_results
    ad_success_rates = []
    facility_costs = []
    air_defense_costs = []
    ...
    df_melted = df.melt(id_vars=["Suksessrate for luftvern"],
    ...
    chart = alt.Chart(df_melted).mark_line(
        ...
        tooltip=["Suksessrate for luftvern", "Kostnadstype", "Kostnad"]
    )
    st.altair_chart(chart)